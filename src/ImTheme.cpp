...
//
// See ImTheme.h for the list of namespace adaptations made in this copy.
//
#include "ImTheme.h"
...
{
    // -------------------------------------------------------------------------
    // ThemeChanged callback hook (replaces upstream HelloImGui RunnerParams)
    // -------------------------------------------------------------------------
    static std::function<void()> g_ThemeChangedCallback;

    void SetThemeChangedCallback(std::function<void()> cb)
    {
        g_ThemeChangedCallback = std::move(cb);
    }
    ...
    namespace ThemeTweakImpl
    {
        ...
        // Custom themes reset via ImGuiStyle{} (StyleColorsDark) then override
        // ImGuiCol_Text for light palettes. InputTextCursor is left at the dark-
        // theme default (white), so the caret disappears on white FrameBg.
        void SyncInputTextCursor(ImGuiStyle& style)
        {
    ...
    ThemeInfo gThemeInfos[] = {
        {Theme_ImGuiColorsClassic,    "ImGuiColorsClassic",  ThemesImpl::ImGui_StyleColorsClassic },
        {Theme_ImGuiColorsLight,      "ImGuiColorsLight",    ThemesImpl::ImGui_StyleColorsLight },
        {Theme_ImGuiColorsDark,       "ImGuiColorsDark",     ThemesImpl::ImGui_StyleColorsDark},
        {Theme_MaterialFlat,          "MaterialFlat",        ThemesImpl::MaterialFlat},
        {Theme_PhotoshopStyle,        "PhotoshopStyle",      ThemesImpl::PhotoshopStyle},
        {Theme_GrayVariations,        "GrayVariations",        [](){return ThemesImpl::ShadesOfGray(3.f, 1.f, 1.f);}},
        {Theme_GrayVariations_Darker, "GrayVariations_Darker",  [](){return ThemesImpl::ShadesOfGray(3.f, 1.136f, 0.865f);}},
        {Theme_MicrosoftStyle,        "MicrosoftStyle",      ThemesImpl::MicrosoftStyle},
        {Theme_Cherry,                "Cherry",              ThemesImpl::Cherry},
        {Theme_Darcula,               "Darcula",          [](){return    ThemesImpl::Darcula();}},
        {Theme_DarculaDarker,         "DarculaDarker",       ThemesImpl::DarculaDarker},
        {Theme_LightRounded,          "LightRounded",        ThemesImpl::LightRounded},
        {Theme_SoDark_AccentBlue,     "SoDark_AccentBlue",   [](){return ThemesImpl::SoDark(0.548f);}},
        {Theme_SoDark_AccentYellow,   "SoDark_AccentYellow", [](){return ThemesImpl::SoDark(0.163f);}},
        {Theme_SoDark_AccentRed,      "SoDark_AccentRed",    [](){return ThemesImpl::SoDark(0.f);}},
        {Theme_BlackIsBlack,          "BlackIsBlack",        ThemesImpl::BlackIsBlack},
        {Theme_WhiteIsWhite,          "WhiteIsWhite",        ThemesImpl::WhiteIsWhite},
    };

    const char* Name(Theme_ theme)
    {
        IM_ASSERT((theme >= 0) && (theme < Theme_Count));
    ...
    Theme_ FromName(const char* themeName)
    {
        ...
        return Theme_ImGuiColorsClassic;
    }
    ...
    ImGuiStyle ThemeToStyle(Theme_ theme)
    {
        ...
        for (size_t i = 0; i < IM_ARRAYSIZE(gThemeInfos); ++i)
        {
            if (gThemeInfos[i].Theme == theme)
            {
                // Reset the live ImGuiStyle to factory defaults before
                // invoking the theme lambda. Most theme implementations do
                // `auto& style = ImGui::GetStyle()` and only overwrite the
                // fields they care about (colours, sometimes a subset of
                // rounding / padding). Without this reset, untouched fields
                // inherit whatever was in `ImGui::GetStyle()` previously —
                // typically scaled metrics from `ImGui::ScaleAllSizes()` —
                // which causes `CaptureBaseStyle()` to snapshot a scaled
                // state as the new "unscaled" baseline. The next
                // `ApplyScale()` then re-scales already-scaled metrics, so
                // window padding / item spacing grow on every theme change.
                ImGui::GetStyle() = ImGuiStyle{};
                ImGuiStyle style = gThemeInfos[i].GetStyleFn();
                ThemeTweakImpl::SyncInputTextCursor(style);
                return style;
            }
        }
        return ImGuiStyle();
    ...
    ImGuiStyle TweakedThemeThemeToStyle(const TweakedTheme& tweaked_theme)
    {
        ...
        const ThemeTweaks& tweaks = tweaked_theme.Tweaks;
    ...
    bool _ShowThemeSelector(Theme_* theme)
    {
        bool changed = false;
        ImVec2 listboxSize =  ImVec2(15.f * ImGui::GetFontSize(), Theme_Count * (ImGui::GetFontSize() + ImGui::GetStyle().ItemInnerSpacing.y) );
        ImGui::Text("Available Themes");
        if (ImGui::BeginListBox("##Available_themes", listboxSize))
        {
            int nbThemes = Theme_Count;
            // We start at 3 because we want to place the older themes at the end
            for (int i = 3; i < nbThemes + 3; ++i)
            {
                Theme_ theme_i = (Theme_)(i % nbThemes);
                const bool is_selected = (*theme == theme_i);
                if (ImGui::Selectable(Name(theme_i), is_selected))
                {
                    changed = true;
                    *theme = theme_i;
                    if (g_ThemeChangedCallback)
    ...
    bool _ShowTweakGui(ThemeTweaks* tweaks)
    {
